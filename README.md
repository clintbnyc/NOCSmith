...
Private, stdio-only MCP access to the official UniFi Network Integration API on pinode. The connector uses the active UniFi OS Server through the Tailscale HTTPS name:

`https://unifi.nutria-newton.ts.net/proxy/network/integration`

It does not use the stopped `/srv/unifi` Docker rollback stack, controller database access, legacy private endpoints, a public listener, or insecure TLS bypasses.
...
- Every API operation must exist in the loaded OpenAPI contract. Arbitrary URLs and methods are rejected.
...
Read tools support the contract's offset, limit, and filter parameters. Page responses include `_connector.truncated`; when true, request another page. A site is auto-selected only if exactly one exists or `UNIFI_DEFAULT_SITE_ID` is configured.
...
The repository vendors Ubiquiti Network OpenAPI `10.3.58`, currently 41 GET and 32 write operations. At startup the connector reads `/v1/info` and probes documented controller-local contract locations. It uses a controller contract only when its version matches the live Network application; otherwise it remains restricted to the reviewed embedded contract and reports the version warning in capabilities and `doctor`.
